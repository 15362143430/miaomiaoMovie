        ...
        <ul>
          <li class="movieList" v-for="item in movieList" :key="item.id">
            <div class="movieBox">
              ...
              <div class="movieInfo">
                <router-link tag="h1" :to="'/info/'+item.id" style="font-size:17px;line-height:24px;margin-top:0">{{item.nm}}</router-link>
              ...
              <div class="movieBuy">
                <p style="color:white;margin-top:3px">预售</p>
              </div>
            </div>
          </li>
        </ul>
...
export default {
  ...
  mounted() {
    this.axios.get("/movieComingList?cityId="+this.id).then(res => {
